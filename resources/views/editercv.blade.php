...
<body>
    <div class="leading-loose">
        <form  action="{{ route('modifierCv',$cv->id) }}"  method="POST" class="max-w-xl m-4 p-10 bg-white rounded shadow-xl mx-auto ">
            @csrf
            @method('PUT')
            <div class="w-full ">
                <div class="grid justify-items-center">
                    <img src="{{asset('/storage/images/'. Auth::user()->avatar )}}" class="w-32 h-32 rounded-full object-center object-cover"/>
                    ...
                    <div class="h-auto w-full mx-auto">
                        <div class="flex flex-col   w-full mx-auto mt-6">
                            <textarea maxlength="250" minlength="50" type="text" name="description" id="description"
                                ...
                                placeholder="Description...">{{ old('name', $cv->description) }} </textarea>
                        </div>
                    </div>
                
          <div class="inline-block mt-2 w-1/2 pr-1">
            <input value="{{ old('name', $cv->numero) }}"  class="w-full px-2 py-2 text-gray-700 bg-gray-200 rounded" id="numero" name="numero" type="text" required="" placeholder="Numero de telephone" >
          </div>
            ...
            <div class="flex flex-col">
                ...
                <div class="relative inline-flex">
                    ...
                    <div class="w-full mx-auto h-auto ">
                        <div >
                            <input value="{{ old('name', $cv->file) }}" class="mt-6 " type="file" id="file" name="file" accept=".pdf">
                        </div>
                    </div>
          <div class="w-full  mt-4 flex">
              <div class="w-auto flex mx-auto">
                    <div>
                        <button class="px-4 py-1 text-white font-light tracking-wider bg-red-500 rounded" type="submit"><a href="{{ route('Cv') }}">Annuler</a></button>
                    </div>
                    <div class="ml-4" >
                        <button type="submit" class="px-4 py-1 text-white font-light tracking-wider bg-blue-500 rounded" type="submit">deposer</button>
                    </div>
              </div>
