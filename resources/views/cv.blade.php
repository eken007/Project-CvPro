                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div>
                            <select
                                ...
                                <option>Semaine derniere</option>
                            </select>
                        </div>
                    </div>
                    @foreach ($cvs as $cv)
                        <div class="mt-6">
                            <div class="max-w-2xl  rounded-md tracking-wide">
                                <div id="header" class="flex">
                                    <div class="w-44 h-44 rounded-full overflow-hidden transform -translate-y-9 translate-x-7">
                                        <img class=" w-full h-full object-cover object-center"
                                            src="{{ asset('storage/images/' . $cv->image) }}" />
                                    </div>
                                    <div id="body" class="flex flex-col ml-5 p-5 w-full bg-white shadow-xl">
                                        <div class="flex w-full">
                                            <div class="relative w-full">
                                                ...
                                                @else
                                                    <div class=" space-x-3 flex absolute right-0 top-0 bottom-0">
                                                        @if ($cv->creator_id == Auth::user()->id)
                                                            <form action="{{ route('supprimerCv', $cv->id) }}" method="POST">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit"
                                                                    class=" min-w-min-content px-2 py-1 bg-red-600 text-gray-100 font-bold rounded hover:bg-blue-600">Supprimer</a>
                                                            </form>
                                                        @endif
                                                        @if ($cv->creator_id == Auth::user()->id)
                                                            <button><a href="{{ route('editeCv', $cv->id) }}"
                                                                    class="px-2 py-1 bg-yellow-400 text-gray-100 font-bold rounded hover:bg-red-500">Editer</a></button>
                                                        @endif
                                                        @can('employee-admin')
                                                            @if ($cv->booster == 0)
                                                                <form action="{{ route('deboosterCv', $cv->id) }}" method="POST">
                                                                    @csrf
                                                                    @method('PUT')
                                                                    <button
                                                                        class="px-2 py-1 bg-blue-600 text-gray-100 font-bold rounded hover:bg-blue-400">Booster</button>
                                                                </form>
                                                            @else
                                                                <form action="{{ route('boosterCv', $cv->id) }}" method="POST">
                                                                    @csrf
                                                                    @method('PUT')
                                                                    @if ($cv->booster_id == Auth::user()->id || $cv->creator_id == Auth::user()->id)
                                                                        <button
                                                                            class="px-2 py-1 bg-red-600 text-gray-100 font-bold rounded hover:bg-red-200">Stop</button>
                                                                    @endif
                                                                </form>
                                                            @endif
                                                        @endcan
                                        ...
                                        <p id="job" class="text-gray-800 mt-2 text-justify">{{ $cv->description }}</p>
                                        <div class="w-full h-auto relative ">
                                            <div class="flex mt-5">
                                                <img alt="avatar" class="w-6 " src="{{ asset('images/phoneb.png') }}" />
                                                <p class="ml-3 font-semibold">{{ $cv->numero }}</p>
                                                <p class="ml-3 font-semibold">{{ $cv->categorie }}</p>
                                            </div>
                                            @guest
                                            ...
                                            @else
                                                @if ($cv->creator_id == Auth::user()->id)
                                                    <div>
                                                        <button type="submit"
                                                            class="absolute right-0 top-0 bottom-0 min-w-min-content px-2 py-1 bg-blue-600 hover:bg-blue-300 text-gray-100 font-bold rounded"><a
                                                                href="{{ route('telechargerCv', $cv->id) }}">Telechager</a></button>
                                                    </div>
                                                @else
                        ...
                        <div class="flex flex-col bg-white px-4 py-6 max-w-sm mx-auto rounded-lg shadow-md">
                            <ul>
                                ...
                                <li class="mt-2"><a href="#"
                                        ...
                                        Back-End</a></li>
                                <li class="mt-2"><a href="#"
                                        class="text-gray-700 font-bold mx-1 hover:text-gray-600 hover:underline">- Analyste
                                        Database</a>
                                </li>
                                <li class="mt-2"><a href="#"
                                        class="text-gray-700 font-bold mx-1 hover:text-gray-600 hover:underline">-
                            ...
                            <div class="flex flex-col bg-white px-8 py-6 max-w-sm mx-auto rounded-lg shadow-md">
                                <div class="flex justify-center items-center">
                                    ...
                                    @else
                                        ...
                                        @if ($recent->creator_id == Auth::user()->id)
                                            <a href="{{ route('telechargerCv', $recent->id) }}"
                                                class="px-2 py-1 bg-blue-600 text-sm text-green-100 rounded hover:bg-blue-300">Telecharger</a>
                                        @else
                                            @can('entreprise-admin')
                                                <a href="{{ route('telechargerCv', $recent->id) }}"
                                                    class="px-2 py-1 bg-blue-600 text-sm text-green-100 rounded hover:bg-blue-300">Telecharger</a>
                                ...
                                @if ($recent == true)
                                    ...
                                    <div class="flex justify-between items-center mt-4">
                                        ...
                                        <img alt="avatar" class="w-6 " src="{{ asset('images/phoneb.png') }}" />
                                        <span class="font-light text-sm text-gray-600">{{ $recent->numero }}</span>
                                @endif
                        @endforeach
                    </div>
                    @endif
